...
void measure_update(double ips_x, double ips_y, double ips_yaw) {
    ...
    //for (int i = 0; i < NUM_PARTICLES; ++i) {
        //ROS_INFO("Weight: %f POSE: X: %f Y: %f", weights.at(i), particles[i][0], particles[i][1]);
    //}
    ...
    for (int i = 0; i < NUM_PARTICLES; ++i) {
        ...
        //ROS_INFO("Particle: %d Weight: %f POSE: X: %f Y: %f", i, weights.at(i), particles[i][0], particles[i][1]);
    }
    //exit(-1);
...
void predication_update(double v, double omega, double dt) {
    ...
    for(int i = 0; i <  NUM_PARTICLES; i++) {
        particles[i][0] = particles[i][0] + v * cos(particles[i][2]) * dt + var_nor();
        particles[i][1] = particles[i][1] + v * sin(particles[i][2]) * dt + var_nor();
        particles[i][2] = particles[i][2] + omega + var_nor();
...
void init_particles(){
    for(int i = 0; i < NUM_PARTICLES; i++){
        particles[i][0] =  (double)((rand() % 100) / 10);
        particles[i][1] =  (double)((rand() % 100) / 10);
        particles[i][2] =  (double)((rand() % 628318) / 100000);
    }
}
